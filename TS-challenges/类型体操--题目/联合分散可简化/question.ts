...
type CamelcaseUnion<Item extends string> = Item extends Item
  ? Camelcase<Item>
  : Item;
...
type As = string | number extends string ? true : false; //false
type TestAs<T> = T extends string ? true : false;
type asd = TestAs<string | number>; //boolean
type TestAsBol<T> = T extends false ? 1 : 2;
type asdd = TestAsBol<boolean>; //1|2 boolean会触发分布式计算
type qs = boolean extends false ? 1 : 2; //2 单独就不会触发
export {};
