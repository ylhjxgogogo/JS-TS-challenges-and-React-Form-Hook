...
type IsEqualRes2 = IsEqual<boolean, false>;
...
type IsTuple<T> = T extends [...arg:infer Args]
?
...
type UnionToIntersection<U> = 
    (U extends U ? (x: U) => unknown : never) extends (x: infer R) => unknown
        ? R
        : never
//U extends U ? (x: U) => unknown : never 必须在生成所有联合类型成员的函数类型
type UnionToIntersection2<U> = 
     (U extends U ?(x:U)=>unknown:never) extends (x:infer Arg)=>unknown
     ? Arg
     :never

type UnionToIntersectionResult=UnionToIntersection2<{guang:1}|{dong:2}>
//GetOptional 得到可选属性
...
type RemoveIndexSignature<Obj extends Record<string, any>> = {
  [k in keyof Obj as k extends `${infer Str}`?Str:never]:Obj[k]
}
